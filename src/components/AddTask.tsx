...
export function AddTask() {
  ...
  function handleChangeNewTaskChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    setTimeout(() => {
      const tasksListFilteredByNewTask = tasks?.filter(task => task.item.toLowerCase().trim() === newTaskTyping.toLowerCase().trim());
  ...
  function handleCreateNewTask(event: FormEvent) {
    ...
    const tasksListFilteredByNewTask = tasks?.filter(task => task.item.toLowerCase().trim() === newTask.toLowerCase().trim());
    if (!newTask || (tasksListFilteredByNewTask && tasksListFilteredByNewTask.length > 0)) {
      return;
    }
  ...
  return (
    <form onSubmit={handleCreateNewTask} className="flex justify-center gap-2 mb-16 -mt-8">
      <Input
